...
const serviceAccount = require(path.join(__dirname, '../thrive-465618-firebase-adminsdk-fbsvc-57bc407cdc.json'));
...
// Use the pulse database
db.settings({ ignoreUndefinedProperties: true });

// Helper function to create a post
async function createPost(postData) {
  const postId = db.collection('discovery_posts').doc().id;
  const post = {
  ...
  await db.collection('discovery_posts').doc(postId).set(post);
