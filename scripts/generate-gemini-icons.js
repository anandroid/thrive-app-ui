...
Create a clean, professional, instantly recognizable icon that matches modern medical/wellness app iconography standards.`;
  ...
  async generateIcon(item, outputPath) {
    ...
    try {
      ...
      // Note: Gemini 2.0 Flash doesn't generate images directly like DALL-E
      // This is a placeholder - we'll need to use a different approach
      console.log(`📝 Generated description for ${item.title}`);
      console.log(`💡 Consider using the description with other image generation tools`);
      
      return {
        success: true,
        name: item.name,
        title: item.title,
        description: response.candidates?.[0]?.content?.parts?.[0]?.text || 'Generated description',
        note: 'Gemini 2.0 Flash generates text descriptions, not images directly'
      };
  ...
  async generateCategorySet(categoryName, items) {
    console.log(`\n🎨 Generating ${categoryName} icon descriptions (${items.length} items)...`);
    
    const outputDir = path.join(__dirname, '..', 'public', 'icons', 'ayurvedic', 'gemini-descriptions', categoryName);
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      const result = await this.generateIcon(item, null);
      results.push(result);
      
      // Save description to file
      if (result.success) {
        const descriptionPath = path.join(outputDir, `${item.name}.txt`);
        await fs.writeFile(descriptionPath, result.description);
        console.log(`💾 Saved description: ${path.basename(descriptionPath)}`);
      }
...
async function main() {
  ...
  const apiKey = process.env.THRIVE_GEMINI_API_KEY || 'AIzaSyCIaqEQ9WdOgiDcsaQJ1B1otzTgYIdIIbs';
  ...
  try {
    console.log('🎯 Gemini Icon Description Generator');
    console.log('═'.repeat(50));
    console.log('📝 Note: Gemini generates descriptions, not images directly');
    console.log('💡 Use these descriptions with other image generation tools\n');
    ...
    switch (command) {
      case 'sample':
        ...
        for (const sample of samples) {
          await generator.generateIcon(sample, null);
      ...
      default:
        console.log('Usage:');
        console.log('  node generate-gemini-icons.js sample    # Generate 4 disease samples');
        console.log('  node generate-gemini-icons.js diseases # Generate all disease descriptions');
        console.log('  node generate-gemini-icons.js herbs    # Generate herb descriptions');
        console.log('  node generate-gemini-icons.js exercises # Generate exercise descriptions');
        console.log('  node generate-gemini-icons.js therapies # Generate therapy descriptions');
        console.log('  node generate-gemini-icons.js all      # Generate all categories');
    }
...
module.exports = { GeminiIconGenerator, ICON_DEFINITIONS, THEME_COLORS };